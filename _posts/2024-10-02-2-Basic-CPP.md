...
_Figure 2.1.A: Working with File I/O Mind Map_{: Legenda}
...
_Code 2.1.B: Sample code to reading and writing files using command line arguments._{: legenda}
...
_Figure 2.1.B: Reading lines Section Mind Map_{: legenda}
...
_Code 2.1.B: How to use a buffer to make reading files more efficient._{: legenda}
...
While standard file I/O operations serve well for basic needs, they can become a bottleneck when dealing with large datasets common in competitive programming. This is where manual buffering comes into play, offering a way to significantly improve I/O performance through careful memory management.
...
_Figure 2.1.C: Manual Buffering Mind Map_{: legenda}
...
_Code 2.1.C: Reading files using buffers._{: legenda}
...
_Figure 2.1.D: Using `mmap` Mind Map._{: legenda}
...
_Figure 2.1.E: `mmap` maps data from physical devices to virtual memory_{: legenda}
...
_Code 2.1.D: Using mmap for faster file I/O._{: legenda}
...
_Figure 2.1.F: Asynchronous I/O Mind Map._{: legenda}
...
![shows two paths one for the main thread and another for the io asynchronou thread](/assets/images/IOAsynchro.webp)

_Figure 2.1.G: Diagram of the asynchronous I/O operations showing a support threading runnib only for I/O_{: legenda}
...
_Code 2.1.E: Using asynchronous I/O in C++20 for deal with big data files_{: legenda}
...
_Code 2.1.F: Mixing `mmap` and asynchronous I/O._ {: legenda}
...
_Figure 2.1.H: The two circular buffers in `io_uring` algorithm._{: legenda}
...
_Code 2.1.G: Using `io_uring` for asynchronous I/O in Linux_. {: legenda}
...
_Figure 2.1.I: Efficient Bulk Data I/O Mind Map._{: legenda}
...
_Figure 2.1.J: Mind map of error handling._{: legenda}
...
**Avoid expensive error checking inside tight loops to improve performance**:
...
_Code 2.1.H: Handling I/O faster with C++._ {: legenda}
...
In competitive programming, `scanf` and `printf` are commonly used due to their efficiency and familiarity. Programmers must ensure they handle inputs correctly to avoid potential issues. **Never, ever, under any circumstances, for any reason, use scanf or `printf` in professional code.** Now that you know the risks, let’s look at an example.
...
_Code 2.1.I: Using `scanf` and `printf` for performance._ {: legenda}
...
_Code 2.1.J: Using even more unsecure functions in performance quest._{: legenda} 
...
`printf_unlocked`, `scanf_unlocked`, work only on Linux because they are part of `glibc`(GNU C Library), the GNU C Library. They skip safety checks to be fast, which is why Windows doesn’t include them. Windows libraries stick to safer, thread-safe options. But you can still use these functions on Windows with compilers like Cygwin or MinGW. They bring Unix-like tools, including glibc, to Windows, letting you use these faster functions.
...
_Code 2.1.K: Fastest I/O functions._{: legenda}
...
## 2.9 Data Manipulation with `std::ranges`
...
## 2.16. Standard Library Algorithms
...
## 2.17 Summary Table of Useful Loop Techniques
